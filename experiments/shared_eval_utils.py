...
from experiments.marl_utils import make_shared_world_env, predict_shared_actions
...
def eval_shared_model(
    ...
    *,
    ...
    return {
        ...
        # Backward-compatible alias with the previous shared eval script.
        "mean_team_return": float(np.mean(world_reward_returns)),
    }


def summarize_shared_results(label: str, results: list[dict[str, float]]) -> None:
    metrics = [
